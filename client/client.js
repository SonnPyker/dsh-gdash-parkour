  ...
  factory: (require) => {
    ...
    exports.apply = function(ctx) {
      ...
      function updatePhysics() {
        ...
        for (const c of colliders) { if (rectsOverlap(nx, player.y, player.w, player.h, c)) { if (player.vx > 0) nx = c.left - player.w; else if (player.vx < 0) nx = c.right; player.vx = 0; break; } }
        player.x = nx;
        ...
        for (let s=0; s<steps; s++){
          ...
          let hit = null;
          for (const c of colliders) {
            if (rectsOverlap(player.x, testY, player.w, player.h, c)) { hit = c; break; }
          }
          if (hit) {
            if (vy > 0) {
              nextY = hit.top - player.h;
              landed = true; landX = player.x + player.w / 2; landY = hit.top;
              vy = 0; player.onGround = true;
              break;
            } else if (vy < 0) {
              nextY = hit.bottom; vy = 0;
              break;
            }
          } else {
            nextY = testY;
